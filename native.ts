...
const firstNamesArray: string[] = members.map(m => m.name.split(" ")[0]);
...
const ageFilter = members.filter(m => {
    m.age !== undefined && m.age >= 41 && m.age <= 60
});
...
var personWithMaxAge = members.reduce((prev: User, curr: User) => {
    if (prev.age && curr.age && prev.age > curr.age) return prev;
    return curr;
}, members[0]);
...
const res = members.reduce((prev: { 'young': string[], 'old': string[], 'noage': string[] }, curr) => {
    if (curr.age) {
...
const result1 = members.reduce((prev: User, curr: User) => {
